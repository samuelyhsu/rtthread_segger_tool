...
static rt_err_t _rtt_cfg(struct rt_serial_device *serial,
...
static rt_size_t mcu_transmit(struct rt_serial_device *serial, rt_uint8_t *buf,
                              rt_size_t size, rt_uint32_t tx_flag) {
...
static struct rt_uart_ops _jlink_rtt_ops = {_rtt_cfg, _rtt_ctrl, _rtt_putc,
                                            _rtt_getc,
#ifdef RT_USING_SERIAL_V2
...
int rt_hw_jlink_rtt_init(void) {
  ...
  SEGGER_RTT_ConfigUpBuffer(RTT_DEFAULT_BUFFER_INDEX, "RTTUP", NULL, 0,
                            SEGGER_RTT_MODE_NO_BLOCK_SKIP);

  SEGGER_RTT_ConfigDownBuffer(RTT_DEFAULT_BUFFER_INDEX, "RTTDOWN", NULL, 0,
                              SEGGER_RTT_MODE_NO_BLOCK_SKIP);
...
// if use RT_USING_SERIAL_V2 you can only use console after schedule, the root
// log missing
#endif
